...
ggsave("results/stl-total.pdf", plot_metro_combined, device = "pdf", width = 5, height = 5)
...
ggsave("results/stl-counties.pdf", plot_counties_combined, device = "pdf", width = 6.5, height = 6)
...
# case_cutoff <- 5
data_for_plot <- data_counties %>%
	filter(!is.na(cases_next) & !is.na(cases_prev)) %>% 
		# (cases_next > case_cutoff) & (cases_prev > case_cutoff)
	filter(date > ymd("2020-03-26")) %>% 
	filter(county %in% c(
		"St. Louis County", "St. Louis City", "St. Charles County", 
		"St. Clair County", "Madison County", # "Monroe County", 
...
plot_stepwise <- ggplot(data = data_for_plot) + 
	...
	annotate(
		...
		size = 2.5, color = grey(0.5), angle = 44,
		x = 5, y = 5, hjust = 0, vjust = 1.5
	) + 
